...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.sz2x7.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const verifyAdmin = async(req,res,next) =>{
      ...
      const requestEmail = await userCollection.findOne({ email: requester });
...
app.get("/", (req, res) => {
  res.send("Doctors Portal is running from server");
});
